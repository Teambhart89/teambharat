...
function avdesh_float_contact() {
	...
	<div class="float-contact" aria-label="Quick contact">
		<?php if ( $whatsapp ) : ?>
			<a class="fc-wa" href="https://wa.me/<?php echo esc_attr( $whatsapp ); ?>" target="_blank" rel="noopener" aria-label="Chat on WhatsApp"><?php echo avdesh_whatsapp_svg( 30 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><span class="tip">Chat on WhatsApp</span></a>
		<?php endif; ?>
...
function avdesh_whatsapp_svg( $size = 20 ) {
	return '<svg width="' . intval( $size ) . '" height="' . intval( $size ) . '" viewBox="0 0 32 32" fill="currentColor" aria-hidden="true" focusable="false">'
		. '<path d="M16.04 4C9.42 4 4.03 9.38 4.03 16c0 2.12.55 4.19 1.6 6.02L4 28l6.13-1.6a12 12 0 0 0 5.9 1.5h.01c6.62 0 12-5.38 12-12S22.66 4 16.04 4zm0 21.9h-.01a9.9 9.9 0 0 1-5.05-1.38l-.36-.22-3.64.95.97-3.55-.24-.37a9.9 9.9 0 0 1-1.52-5.28c0-5.48 4.46-9.94 9.95-9.94 2.66 0 5.15 1.04 7.03 2.92a9.88 9.88 0 0 1 2.91 7.03c0 5.48-4.46 9.94-9.94 9.94zm5.45-7.44c-.3-.15-1.77-.87-2.04-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.17-.17.2-.35.22-.65.07-.3-.15-1.26-.46-2.4-1.48-.89-.79-1.49-1.77-1.66-2.07-.17-.3-.02-.46.13-.61.14-.14.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.07-.15-.67-1.62-.92-2.22-.24-.58-.49-.5-.67-.51-.17-.01-.37-.01-.57-.01s-.52.07-.8.37c-.27.3-1.04 1.02-1.04 2.48s1.07 2.88 1.22 3.08c.15.2 2.1 3.2 5.08 4.49.71.31 1.26.49 1.69.63.71.22 1.36.19 1.87.12.57-.09 1.77-.72 2.02-1.42.25-.7.25-1.3.17-1.42-.07-.12-.27-.2-.57-.35z"/>'
		. '</svg>';
}

/** WhatsApp click-to-chat URL from the saved number (empty string if unset). */
function avdesh_whatsapp_url() {
	$num = preg_replace( '/[^0-9]/', '', avdesh_opt( 'avdesh_whatsapp', '' ) );
